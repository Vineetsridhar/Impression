...
def on_new_connection(data):
    for connection in db.session.query(tables.Connections).filter((tables.Connections.user1_email == data["user1_email"]) & (tables.Connections.user2_email == data["user2_email"])).all():
        return -1
    for connection in db.session.query(tables.Connections).filter((tables.Connections.user1_email == data["user2_email"]) & (tables.Connections.user2_email == data["user1_email"])).all():
        return -1
...
def on_query_connections(data):
    ...
    for connection in db.session.query(tables.Connections).filter((tables.Connections.user1_email == data["user_email"]) | (tables.Connections.user2_email == data["user_email"])).all():
